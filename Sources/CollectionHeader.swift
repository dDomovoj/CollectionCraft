...
public protocol ICollectionHeader: IIdentifiable {
...
public struct CollectionHeader<T: UICollectionReusableView & IReusableCell>: ICollectionHeader {
    ...
    // MARK: - ICollectionHeader

    public func register(in containerView: Container) {
        T.register(in: containerView, kind: UICollectionView.elementKindSectionHeader, reuseIdentifier: reuseIdentifier)
    }

    public func instance(for containerView: Container, at indexPath: IndexPath) -> UICollectionReusableView {
        return containerView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader,
...
extension CollectionHeader where Data: IIdentifiable {

    var id: Self.ID { data.id }

}

extension CollectionHeader where Data: ExpressibleByNilLiteral {
...
extension CollectionHeader where Data == Void {
